...
critic_path = '/home/skleff/misc_repos/arm/trained_models/dvp/Order_1/Horizon_200/eps_19.pth'
...
# Plot results
fig, ax = utils_amit.plot_ddp_results(DDPS, robot, SHOW=False, sampling_plot=1)
for i in range(nq):
    ax['x'][i,0].plot(np.linspace(0,N_h*dt, N_h+1), [q_max[i]]*(N_h+1), 'ro')
    ax['x'][i,0].plot(np.linspace(0,N_h*dt, N_h+1), [q_min[i]]*(N_h+1), 'ro')
    ax['x'][i,1].plot(np.linspace(0,N_h*dt, N_h+1), [v_max[i]]*(N_h+1), 'ro')
    ax['x'][i,1].plot(np.linspace(0,N_h*dt, N_h+1), [v_min[i]]*(N_h+1), 'ro')
plt.show()
